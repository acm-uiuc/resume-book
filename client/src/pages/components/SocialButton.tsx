...
const LinkedIn_svg = () => (
	<>
		<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-4" fill="currentColor" viewBox="0 0 24 24">
...
const icons: { [key: string]: { name: string; svg: FC; color: string; link: string } } = {
	linkedin: {
		name: "LinkedIn",
		svg: LinkedIn_svg,
		color: "#0077b5",
...
const SocialButton: FC<SocialButtonProps> = (props) => {
	...
	return (
		<>
			<button
				onClick={() => {
					if (icons[type].link.includes("mailto:")) location.href = icons[type].link;
					else
						window.open(
							icons[type].link,
							"_blank" // new window.
