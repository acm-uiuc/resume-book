...
from util.s3 import (
    create_presigned_url_for_put,
    create_presigned_url_from_s3_url,
    async_generate_presigned_urls,
)
...
extra_origins = os.environ.get(
    "ValidCorsOrigins", "https://resumes.acm.illinois.edu"
).split(",")
...
def shared_get_profile(username):
    ...
    if profile_data and "resumePdfUrl" in profile_data:
        profile_data["resumePdfUrl"] = create_presigned_url_from_s3_url(
            s3_client, profile_data["resumePdfUrl"]
        )
    return Response(
...
def student_gpt():
    ...
    try:
        ...
        if len("".join(data["roleKeywords"])) > 300:
            return Response(
                ...
                content_type=content_types.APPLICATION_JSON,
                body={
                    "message": "Error validating profile generation payload",
                    "details": "Role keywords are too long.",
                },
            )
        response = oai_get_profile_json(
            openai_client,
            data["resumeText"],
            data["roleType"],
            ",".join(data["roleKeywords"]),
        )
        ...
        for prop in url_properties:
            if (
                response[prop] != ""
                and "http://" not in response[prop]
                and "https://" not in response[prop]
            ):
                response[prop] = f"http://{response[prop]}"
        response["resumePdfUrl"] = create_presigned_url_from_s3_url(
            s3_client, f"s3://{S3_BUCKET}/resume_{username}.pdf"
        )
    except pydantic.ValidationError as e:
        return Response(
            status_code=403,
    ...
    try:
        response = json.loads(
            StudentProfileDetails(**response).model_dump_json(), parse_float=Decimal
        )
    except pydantic.ValidationError as e:
        return Response(
            status_code=403,
        ...
        return Response(
            ...
            content_type=content_types.APPLICATION_JSON,
            body={
                "message": "Error performing profile generation (post OAI call)",
                "details": str(e),
            },
        )
    return Response(
    ...
    try:
        ...
        with db_connection.transaction():
            with db_connection.cursor(row_factory=dict_row) as cur:
                ...
                search_result = [
                    convert_dict_keys_snake_to_camel(x) for x in cur.fetchall()
                ]
    except pydantic.ValidationError as e:
        return Response(
            status_code=403,
...
def recruiter_perform_download():
    json_body: dict = app.current_event.json_body or {}
    urls = []
    try:
